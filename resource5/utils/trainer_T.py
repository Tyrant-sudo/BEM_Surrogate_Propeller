...
config = {
    ...
    'save_path': '/home/sh/WCY/auto_propeller/resource5/1_model/Thrusttest_model.ckpt',  # Your model will be saved here.
    'loss_history_path': '/home/sh/WCY/auto_propeller/resource5/1_model/Loss_Thrusttest_model.json'
}
...
def trainer(train_loader, valid_loader, model, config, device):
    ...
    optimizer = torch.optim.Adam(model.parameters(), lr=config['learning_rate'], betas=(0.9, 0.999), eps=1e-08, weight_decay=0.0, amsgrad=False)
...
if __name__ == "__main__":
    ...
    df = pd.read_csv(Thrust_data)

    # 解析输入列
    train_x_col1 = parse_float_array_from_df_column(df[head_Thrust[1]])
    
    x0  = parse_float_array_from_df_column(df[head_Thrust[0]])
    x,y = parse_float_array_from_df_column(df[head_Thrust[2]]),parse_float_array_from_df_column(df[head_Thrust[3]])*np.pi/180
    ...
    # 选择输出列
    train_y = np.array(df[[head_Thrust[4], head_Thrust[7]]].values.tolist())
