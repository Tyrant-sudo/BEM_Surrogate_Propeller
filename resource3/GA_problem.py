...
class constraint:
    ...
    def __init__(self,config_path):
        ...
        self.lb    = cfg.getfloat('cacl','lower_b')
    ...
    def get_len(self,len_sec):
        ...
        self.n_var = len_sec + len_sec
        self.n_obj = 1 #效率最大(悬停或者有前进速度)
        self.n_ieq_constr = 2 + get_cons(self.local_cl)+get_cons(self.local_cR)+get_cons(self.local_pitch)
        #三个截面限制+最小值限制（两个）
...
class Optimize_BestEta_ConsTPitchCrCl(Problem):
    ...
    def get_base(self):
        ...
        t_cR,c_cR,_ = get_Bspline(self.radius,self.c_R,k=4)
        self.t_cR     = np.expand_dims(t_cR,0).repeat(x.shape[0],axis=0)
        self.c_cR     = np.expand_dims(c_cR,0).repeat(x.shape[0],axis=0)
        ...
        draw_block(self.radius,self.c_R,self.pitch*180/np.pi,self.save_path,'base',self.base_eta,self.base_T)
    ...
    def _evaluate(self, x, out, *args, **kwargs):
        ...
        c_cR      = np.add(x[:,cur_p:cur_p+len1]*para1,self.c_cR)
        c_cR      = np.concatenate((c_cR,self.tail0),1)

        
        pitch_x  = reduce_B(self.radius,self.t_pitch,c_pitch)

        cR_x     = reduce_B(self.radius,self.t_cR,c_cR)
        
        eta,T,g_cl_list,g_pitch_list,g_cR_list = \
        get_eta(pitch_x,cR_x,self.s,self.c)
        
        if self.s.v_inf >eps:
            process_eta.append(np.mean(eta)/100)
            draw_block(self.radius,cR_x[0],pitch_x[0]*180/np.pi,self.save_path,str(0),eta[0]/100,T[0],False)
        else:
            process_eta.append(np.mean(eta))
            draw_block(self.radius,cR_x[0],pitch_x[0]*180/np.pi,self.save_path,str(0),eta[0],T[0],False)
        process_T.append(np.mean(T))

        draw_process(process_eta,self.base_eta,process_T,self.base_T,self.save_path)
      
        f1 = -100 * eta
        
        g0 = self.min_eta - np.array(eta)
        
        g1 = 0.01*(self.min_T  -  np.array(T))
        g2 = 100*(np.array( g_cl_list) - eps)
        g3 = 100*(np.array(g_pitch_list) - eps)
        g4 = 100*(np.array(g_cR_list) - eps)
        
        if get_cons(self.c.local_cl)==0: 
            g = np.column_stack([g0,g1,g3,g4])
        else:
            g = np.column_stack([g0,g1,g2,g3,g4])
    ...
    def draw_result(self,result_file,save_file):
        ...
        for i in range(len(pitch_x)):
            print(i)
            if i%1 == 0:
                draw_block(self.radius,cR_x[i],pitch_x[i]*180/np.pi,save_file,i,eta[i],T[i])
...
class Optimize_BestEta_ConsPitchCl(Problem):
    ...
    def __init__(self, s:Solver,c:constraint,save_path):
        ...
        self.n_var = c.n_var
        self.n_obj = c.n_obj
        self.n_ieq_constr = get_cons(c.local_cl)+get_cons(c.local_pitch)
        self.c  = c
        self.save_path = save_path
    ...
    def _evaluate(self, x, out, *args, **kwargs):
        ...
        g3 = 0*(np.array(g_pitch_list) - eps)
        g4 = 0*(np.array(g_cR_list) - eps)
        
        g = np.column_stack([g3,g4])
        
        np.save(self.c.result_path+'x',x)
        np.save(self.c.result_path+'process_eta', np.array(process_eta))
...
if __name__ == '__main__':
    geom_path = 'example_OPTIMIZE/prop1.ini'
    res_path ='example_OPTIMIZE/restrict1.ini'
    result_path  = 'example_OPTIMIZE/result/x.npy'
    pic_path  = 'example_OPTIMIZE/result_fig/'
    save_path = 'example_OPTIMIZE/process/'
