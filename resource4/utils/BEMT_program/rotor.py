...
from .airfoil import load_airfoil,cacl_airfoil
import warnings
...
class Rotor: 
    ...
    """
    def __init__(self, cfg, name, mode,s1=[],r1=[],c1=[],p1=[]):
        ...
        if len(p1)==0:
            pitch = cfg.get(name, 'pitch').split()
        else:
            pitch = p1
            
        try:
            load_path = cfg.get(name,'load_path')
        except:
            load_path = ''
        ...
        self.alpha = [float(p) for p in pitch]
        self.sections = []
        for i in range(self.n_sections): 

            if s[i] == "coordinate":

                sec = Section(s1[i], float(r[i]), float(dr[i]), radians(self.alpha[i]), float(c[i]),self, mode, True)
            else:
                sec = Section(load_airfoil(s[i],load_path), float(r[i]), float(dr[i]), radians(self.alpha[i]), float(c[i]),self, mode, False)
    ...
    def sections_dataframe(self):
        ...
        return pd.DataFrame(data) #返回一个表格
...
class Section: 
    ...
    :param string mode: Solver mode
    """
    def __init__(self, airfoil, radius, width, pitch, chord, rotor, mode, xfoil = False):
        ...
        self.alpha_range = 20.0
        
        if xfoil:
            
            self.airfoil = cacl_airfoil(airfoil,self.alpha_range, self.Re)
        else:
