...
class Solver: 
    ...
    def __init__(self, config_path,s1=[],r1=[],c1=[],p1=[]):
        ...
        if cfg.has_section('turbine'):
            self.mode = 'turbine'
            self.rotor = Rotor(cfg, 'turbine', self.mode,s1=s1,c1=c1,r1=r1,p1=p1)
        else:
            self.mode = 'rotor'
            self.rotor = Rotor(cfg, 'rotor', self.mode,s1=s1,c1=c1,r1=r1,p1=p1)
    ...
    def run_sweep(self, parameter, n, low, high):
        ...
        for i,p in enumerate((np.linspace(low, high, n))):
            setattr(self, parameter, p) #创建类属性
