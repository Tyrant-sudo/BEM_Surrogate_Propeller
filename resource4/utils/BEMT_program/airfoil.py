...
def load_airfoil(name,load_path):
    """
    ...
    try:
        try:
            # print(path)
            a.alpha_, a.Cl_, a.Cd_,_ = np.loadtxt(path, skiprows=14, unpack=True)
        except:
            a.alpha_, a.Cl_, a.Cd_,_ = np.loadtxt(load_path+name, skiprows=14, unpack=True)
    except:
        a.alpha_, a.Cl_, a.Cd_,_ = np.loadtxt(load_path+"extrapolate_"+name, skiprows=14, unpack=True)
...
def cacl_airfoil(coordinate,alpha,Re):
    
    sys.path.append('/home/sh/WCY/auto_propeller/resource4/utils/extrapolate_program')
    
    from deal_extrapolate import get_extrapolate

    a = Airfoil()
    
    af,_ = get_extrapolate(coordinate,alpha,Re)
    a.alpha_, a.Cl_, a.Cd_ = af.alpha, af.cl, af.cd
    
    # Checking for and handling duplicates
    unique_alpha, index = np.unique(a.alpha_, return_index=True)
    if len(unique_alpha) != len(a.alpha_):
        # Duplicates detected
        # Option 1: Remove duplicates (simple method)
        a.alpha_ = unique_alpha
        a.Cl_ = a.Cl_[index]
        a.Cd_ = a.Cd_[index]
